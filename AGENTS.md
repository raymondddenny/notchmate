...
- `Lyrics/` - `LyricsController` (ObservableObject; fetches from LRCLIB via URLSession+Codable, no API key; caches per trackID; parses LRC `[mm:ss.xx]` timestamps; polls Spotify `player position` every 1s via AppleScript on a dedicated serial queue; publishes `LyricsState` with `.synced(lines:currentIndex:)`, `.plain(text:)`, `.noMatch`, `.loading`, `.idle`; line sync works Spotify source only - system source shows lyrics without advancing) and `LyricsWidget` (expanded only; synced: fixed 5-line window centered on current line with depth-fade opacity; plain: scrollable block; no-match/loading: status row). Free-tier, no license gate.
